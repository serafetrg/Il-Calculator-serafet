...
def _compute_composition_no_delta(lp_state: LPState, price: float, side_label: str) -> CompositionPoint:
    ...
    return CompositionPoint(price, side_label, 0.0, sol_amt, usdc_amt, sol_pct, usdc_pct, lp_val)
...
def generate_data(lines: int, deposit: float, anchor: float, lower: float, upper: float):
    ...
    for price in uniq_prices:
        ...
        points.append(_compute_composition_no_delta(lp_state, price, side))
...
st.markdown("Calculates the optimal short distribution to hedge an Anchor -> Lower Bound LP position.")
...
with st.sidebar:
    ...
    with col2:
        UPPER_PCT = st.number_input("Upper Range (%)", value=10.0, step=0.5) # Used for L calculation
...
if points and lp_state:
    ...
    st.dataframe(
        df.style.apply(highlight_small_orders, axis=1).format({
            "Price": "{:.4f}",
            "SOL Δ (Size)": "{:.6f}",
            "Order Value ($)": "{:.2f}",
            "Total SOL": "{:.4f}",
